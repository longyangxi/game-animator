...
// 회귀 테스트(B1/B2): 몸통은 4프레임 모두 동일(같은 크기·같은 수직 위치·발 고정),
// 오직 검 길이만 프레임마다 다르게 한다. 안정적인 슬라이서라면 출력 4프레임에서
// "몸통"의 위치/크기가 ±1px 이내로 동일해야 한다. 수정 전에는 B1(스케일 맥동)/
// B2(수평 흔들림)로 크게 흔들렸다(wΔ≈13, leftΔ≈22). 공유 스케일 + 굵은-컬럼 앵커로
// 수정한 뒤 Δ=0. 배경: docs/sprite-slicing-stability.md
//
