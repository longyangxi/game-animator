...
func ExtractFrames(strip *image.NRGBA, expected, cellW, cellH, margin int) ExtractResult {
	...
	for _, g := range fcs {
		...
		// bounding box to fit the available space. (Per-frame fit guarantees the full content never
		// clips; the tradeoff is the B1 size pulse, which is the lesser evil vs. clipping the body.)
		...
		// place horizontally so the center of mass lands at the cell center.
